...
extend(Raven.prototype, {
  config: function config(dsn, options) {
	...
	if(options.ignoreErrorNames && options.ignoreErrorNames.length) {
  	  var patternArray = [];
      for (var i=0; i < options.ignoreErrorNames.length; i++) {
          var pattern = options.ignoreErrorNames[i];
          // Taken from: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions
          patternArray.push(pattern.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, '\\$1'));
      }
      this.ignoreErrorNamesRegExp = new RegExp(patternArray.join('|'), 'i');
    }
  ...
  captureException: function captureException(err, kwargs, cb) {
    ...
    if (err.name && this.ignoreErrorNamesRegExp && this.ignoreErrorNamesRegExp.test(err.name)) {
      return;
    }
